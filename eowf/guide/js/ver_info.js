...
var waei	=	'269';  // <- 『英辞郎』データの和英の見出し語数
var waei_fr	=	'';  // <- 『英辞郎』データの和英の見出し語数 端数
var rdate	=	'2015年12月16日';  // <- 『英辞郎』データのバージョン（そのバージョンの EOW のリリース日ではない！）のリリース日を YYYY年MM月DD日 形式で記入
var reibun	=	'57';  // <- 『英辞郎 on the WEB Pro』に含まれる例文データ項目数
var rei_eow	=	'14';  // <- 『英辞郎 on the WEB』に含まれる例文データ項目数
var del		=	'1,185';
...
function showDel() {
	if (del !== '') {
		document.write('「英辞郎 on the WEB Pro」には英和見出項目 ' + eiwa + ' 万件に加え、例文が ' + reibun + ' 万件収録されています。（無料版「英辞郎 on the WEB」に収録されている例文は ' + rei_eow + ' 万件です）<br />また、この例文データには、独立行政法人情報通信研究機構作成の『日英新聞記事対応付けデータ』が、英和対訳、和英対訳の形で、' + del + '件ずつ組み込まれています。');
	};
};
