...
$(function(){
  ...
  $(document).ready(function(){
    ...
    if (canUseSpeech) {
      $("#tabcontent .col04").css("width", "7%");
    } else {
      $("#tabcontent .col04").css("width", "4%");
      $("td.open_close_arrow img.speech").hide();
      $("td.open_close_arrow span").hide();
    }
  });
  //タグ追加
...
//  $('.del_tag').live('click', function(){ ※.liveはjQuery1.9以降削除されているため
  ...
  //訳語表示/非表示
  $("td.toggler:not(img.speech)").click(function(){
    ...
    var img = $(this).closest('tr').children('td:eq(2)').find("img:first");
    var imgSrc = img.attr("src");
    if (imgSrc.indexOf("open")>=0) {
        img.attr("src", imgSrc.replace("open", "close"));
    } else if (imgSrc.indexOf("close")>=0) {
        img.attr("src", imgSrc.replace("close", "open"));
    }
    return false;
  });
  // 音声再生
  $("td.toggler img.speech").click(function(e){
    e.preventDefault();
    word = $(this).closest('tr').children('td:eq(1)').text();
    playSpeech(word);
    return false;
  });
  $('body').click(function(){
  ...
  $('#printer').click(function(){
    ...
    doc.write('<html>');
    doc.write('<head>');
    doc.write('<title>英辞郎 on the WEB Pro</title>');
    doc.write('<link href="/content/css/eowp.css?201312241000" rel="stylesheet" type="text/css">');
...
//    doc.write('<script type="text/javascript" language="JavaScript" src="/content/js/store.min.js?201312241000"></script>');
    ...
    doc.write('<script type="text/javascript" language="JavaScript" src="/content/js/eowp_print.js?201312241000"></script>');
    doc.write('</head>');
...
//              }
        ...
        buttons: {
          "インポート": function(){
        	...
        	.error(function(){
        	  ...
        	  alert("単語帳インポートに時間がかかっております。\n時間をおいて再度単語帳を開きなおして下さい。\n(単語帳インポートは続いております。)");
        	  location.reload(true);
        	});
  ...
  $('.open_memo').click(function(){
    ...
    $('#word_dialog_contents').append('<div class="dialog_desc">≪意味・用例≫<div class="desc">'+desc+"</div></div>");
