...
function validateOptionForAction(action, word) {
  //  [ ] { } " ( ) | ^- 　\s-　記号は使用できない
  var validwd = "\[|\]|\{|\}|\"|\(|\)|\||^-|\s-";
  var chk;
  if (!word instanceof Array) {
  ...
  for (var i = 0; i < chk.length; i++) {
    if (chk[i]) {
      if (chk[i].match(/\[|\]|\{|\}|\"|\(|\)|\||^\-|\s\-/)) {
        alert(action + "では、[ ] { } \" ( ) | -を用いた検索式は使用できません。");
        return false;
      }
    }
  }
...
function goSearch() {
  ...
  // EOW_NEW-1254
  var variant_op = "";
  var searchValue = document.forms[obj].q.value;
  if (a.length > 3) {
  
    if (!validateQuery(searchValue)) return false;
    if (!validateOptionForAction("「変化形を含めて検索」", searchValue)) return false;
    variant_op = "&ref=op";
    searchValue = padGradableMark(searchValue);
  }
...
//    if (REF_SEARCH != "") {ref = "&ref=" + REF_SEARCH;}
    ...
    if (wname != "") {
      // EOW_NEW-116
      window.open(url + "?q=" + encodeURL(toHalfWidth(searchValue)) + ref + variant_op, wname);
    } else {
      document.location.href = url + "?q=" + encodeURL(searchValue) + ref + variant_op;
    }
  }
  return false;
};
...
function goService(sid,dc,q) {
  ...
  if (sid == 1) {
    ...
    var ref = "resrch",
      checkQuery = decodeURL(q)
    wn = WINDOW_NAME;
    if(checkOnlyOneChar(checkQuery)){
      ref = "vl"
    }
    if(!validateQuery(checkQuery)) {
      return false
    } else {
      url = SEARCH_URI + "?q=" + q + "&ref=" + ref;
    }
  } else if (sid == 5) {
    if (dc == "EJ") {
  ...
  if( (window.name == WINDOW_NAME_SMALL || window.name == WINDOW_NAME_APP) && sid == 4 ){
...
function highlightExampleIcon(text, wd) {
  ...
  midashis[2] = replaceAll(midashis[1], " ", "");
  ...
  var isje = false;
  if ($("body.js_je")) {
	  for (var p = 0; p < fulltext.length - 1; p++) {
	  	  if (fulltext[p] == "<") {
	  	  	  // タグの始まりの場合はタグの終わりまでを無視
	  	  	  var e1 = fulltext.indexOf(">", p);
	  	  	  if (e1 > -1) {
	  	  	  	  p = e1;
	  	  	  	  continue;
	  	  	  }
	  	  	  // タグが閉じられていない場合は、とりあえず次の文字へ(HTML的にはエラーですが)
	  	  	  continue;
	  	  }
	  	  // タグの中の場合は、そこからタグの始まりまでを抜き出す
	  	  var e = fulltext.indexOf("<", p);
	  	  if (e == -1) {
	  	  	  e = fulltext.length;
	  	  }
	  	  var innerTextoriginal = fulltext.slice(p, e);
	  	  // 抜き出した中の半角スペースをなくす
	  	  var innerText = replaceAll(innerTextoriginal, " ", "");
	  	  
	  	  // 見出し語にマッチするか？
	  	  var isok = false;
		  for (var i = 0; i < midashis.length; i++ ) {
		    if ( innerText.indexOf(midashis[i]) >= 0 ) {
		      var rep = spanJumpS + fontJumpS + innerTextoriginal + fontJumpE + spanJumpE;
		      fulltext = fulltext.substr(0, p) + rep + fulltext.substr(e);
		      isok = true;
		      break;
		    }
		  }
...
//  var wd = midashi;
  ...
  if (!!window.fulltext) {
    ...
    plainText = escapeHTML(fulltext);
    plainText = plainText.replace(/<br\s*?\/*>/ig, "\n");
...
function checkStopWord(q) {
  const stopWordsList = new Array("a","all","and","are","b","be","by","c","can","d","e","f","for","g","h","he","how","i","in","j","k","l","m","me","n","o","of","p","q","r","s","so","some","t","that","the","to","u","v","w","what","with","x","y","z");
