...
function new_tag(){
  ...
  $.getJSON(JSON_URI+'wordbook/addTag', {'wordType': wordType, 'tag': tag}, function(json){
    if(json.ret == 0){
      ...
      for(var i=0;i<tags.length;i++){
        tags_html += '<tr>';
        tags_html += '<td class="close"><a href="" id="tag_'+tags[i]+'" class="del_tag js_tag_list" onclick="return del_tag('+tags[i]+')"><img src="'+IMG_URI+'bt_del.png" width="40" height="40" alt="削除" title="削除" /></a></td>';
