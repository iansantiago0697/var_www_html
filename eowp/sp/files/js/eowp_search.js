...
//        REF_SEARCH = REF_SEARCH_COLLOCATION;
    ...
    // 検索結果が膨大な検索ワードの場合、ref="vl"を設定し完全一致の検索に変更する
    if(checkStopWord(document.forms[obj].q.value)){
      ref = "vl";
...
// 全文表示のアイコン変更
  ...
  for (var i = 0; i < display_tag.length; i++) {
    if ( display_tag[i].className == "exp" ) {
      ...
      // ラベルの CSS 化
      if (label.match(/^\".+\", \".+\"$/)) {
        ele.innerHTML = "<span class='vat'><a href='javascript:goFullText(" + stripTags(label) + ")' title='全文を表示する'><img src='" + IMG_URI + "ic_read.png' border='0' align='absbottom' width='34' height='25'> <u>全文表示</u></a></span>";
      }
      continue;
    }
    if (display_tag[i].className == "sound") {
      ...
      if (!onseiFile.match(/^<a/)) {
        ...
        onseiHTML += "<img class='play' src='" + IMG_URI + "sound.png' alt='クリックして音声を聞く' />";
        onseiHTML += "</a>";
...
// 検索結果が膨大な検索ワードの確認
// 20180514 かな1文字(半角カナを含む) +
// [EOW_NEW-1146] 該当件数がおおそうな英字(eijiro.M_CACHE_WORD_LIST_JAVA)
function checkStopWord(q) {
  ...
  return (q.match(/^([\u3040-\u309f\u30a0-\u30ff]{1}|[\uff65-\uff9f]{1}[\uff9e\uff9f]?)$/) || stopWordsList.some(function(word){return word == q.toLowerCase()}));
}
