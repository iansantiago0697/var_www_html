//
// **英辞郎 on the WEB Pro お試し版** の広告運用用スクリプト
//
...
(function(global) {
  ...
  // Opera で `window.IMG_URI` が空文字列となり、EOWPF のトップ階層に画像のリクエスト(404となる)
  // が来ていることに対処。
  //
  ...
  mc.helpers.getUIImgUrl = function mc_getUIImgUrl(fileName) {
    ...
    if(typeof baseUrl === 'undefined' || !/^http/.test(baseUrl)) {
       ...
       * テスト環境のURLを返すように変更する。
       */
      baseUrl = document.location.protocol + '//eowpf.alc.co.jp/sp/content/img/';
    }
  ...
  mc.helpers.getContextPath = function mc_getContextPath() {
    return window.CONTEXT_PATH ? window.CONTEXT_PATH : document.location.protocol + '//eowpf.alc.co.jp/';
  };
  ...
  mc.contents.remark = {
    banners: [{
      keywords: '年額コースなら1カ月分お得！',
      url: 'https://home.alc.co.jp/db/owa/sp_sacd_redir_pt?code=c_eowpf_login_02'
    },{
  ...
  mc.templates.remark = [
    ...
    '<div class="remark">',
        '<a href="https://shop.alc.co.jp/spg/cart/renewpass/form/-/" target="_blank">パスワードをお忘れの方はこちら</a>',
        '<hr>',
        '<a href="https://home.alc.co.jp/db/owa/sp_sacd_redir_pt?code=o_eowpf_sp_login_01" target="_blank">「英辞郎 on the WEB Pro」とは？</a>',
        '<a href="<%=bnr.url%>" target="_blank"><%=bnr.keywords%></a>',
        '<a href="/login" onclick="setSpCookie(\"pc\")" class="pc">PC版を表示する</a>',
    '</div>'
  ].join('\n');
  ...
  //
  // `mc.addCSS()` でCSSを設定する。
  // ここで設定する以外にも、 eowpf.css に広告に使われているCSSが含まれている。
  //
  ...
  global.ad_top = function() {
    //TOP広告(自社広告)
    adInfoTop = [
         '<div style="margin-bottom: 40px; margin-right: 10px;">',
         '<a target="_blank" href=" https://eowp.alc.co.jp/info-business/?utm_source=eowf_sp&utm_medium=rectangle&utm_campaign=151015_FTop" >',
         '<img src="/content/img/151015_FTop.png">',
         '</a>',
         '</div>'
    ].join('\n');
    document.write(adInfoTop);
  };

    global.ad_serp_head = function() {
    //SERPヘッダ広告
    adInfoSerpHead = [
         '<script language="javascript" src="https://ds.advg.jp/adpds_deliver/js/pjs.js"></script>',
         '<script language="javascript">adpds_js("https://ds.advg.jp/adpds_deliver", "adpds_site=alcad0505&adpds_frame=smartphone_320_100");</script>'
    ].join('\n');
    document.write(adInfoSerpHead);
  };


  global.ad_serp_footer = function() {
    //SERP footer 広告
    adInfoSerpFooter = [
         '<script language="javascript" src="https://ds.advg.jp/adpds_deliver/js/pjs.js"></script>',
         '<script language="javascript">adpds_js("https://ds.advg.jp/adpds_deliver", "adpds_site=alcad0505&adpds_frame=smartphone_250_300");</script>'
    ].join('\n');
    document.write(adInfoSerpFooter);
  };
  ...
  global.ad_serp_overlay = function() {
    //SERP overlay 広告
    adInfoSerpOverlay = [
      ...
      '<div id="57bbdca2bdfc8b1541fcb0cc3f2ab49a" >',
        '<script type="text/javascript">',
          'microadCompass.queue.push({',
          '"spot": "57bbdca2bdfc8b1541fcb0cc3f2ab49a",',
          '"url": "${COMPASS_EXT_URL}",',
    ...
    document.write(adInfoSerpOverlay);
  };

}(window));

// もしHTMLで定義されなかった場合に備えて定義しておく
window.CONTEXT_PATH = document.location.protocol + '//eowpf.alc.co.jp/';
